...
export class ProductReviewSeeder implements Seedable {
    ...
    private async getRandomModerationStatus(
        manager: EntityManager
    ): Promise<ProductReviewModerationStatus> {
        ...
        return manager.findOneByOrFail(ProductReviewModerationStatus, {
            name: chosenStatus,
        });
    }
    ...
    async seed(manager: EntityManager): Promise<void> {
        await manager.delete(ProductReview, {});

        const newReviews: ProductReview[] = [];

        for (let [productTitle, reviews] of Object.entries(
            ProductReviewSeeder.productReviews
        )) {
            ...
            await Promise.all(
                reviews.map(async (reviewInfo, i) => {
                    const newReview = new ProductReview();
                    newReview.product = await manager.findOneByOrFail(Product, {
                        title: productTitle,
                    });
                    newReview.user = randomUsers[i];
                    newReview.reviewMessage = reviewInfo.review;
                    newReview.starRating = reviewInfo.rating;
                    newReview.moderationStatus =
                        await this.getRandomModerationStatus(manager);
                    newReview.createdAt = this.getRandomReviewDate();
        ...
        await manager.insert(ProductReview, newReviews);
    }
}
