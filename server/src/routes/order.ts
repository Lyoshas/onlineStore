...
import { productsExist } from '../models/product.js';
...
function doesThisNameExist(tableName: string, name: string): Promise<boolean> {
    if (
        !['delivery_methods', 'order_payment_methods', 'cities'].includes(
            tableName
        )
    ) {
        throw new Error(
...
router.post(
    ...
    body('deliveryMethodName')
        ...
        .custom((deliveryMethodName) => {
            return doesThisNameExist('delivery_methods', deliveryMethodName);
        })
    ...
    body('postOffice')
        ...
        .custom(async (postOffice, { req }) => {
            ...
            if (companyName === 'Нова Пошта') {
                ...
                return jsonResult.data.length === 0
                    ? Promise.reject()
                    : Promise.resolve();
...
async function checkOrderId(orderId: number) {
    const storedOrder = await dbPool
        .query(
            `
            SELECT
                o.is_paid,
